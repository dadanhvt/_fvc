(function () {
    ...
    angular
        ...
        .controller('EmpGoogleCampaignController', EmpGoogleCampaignController);

    /* @ngInject */
    function EmpGoogleCampaignController($rootScope, apiService, $mdDialog, $filter, $mdToast) {
        var vm = this;
        ...
        function getCampaigns(){
            apiService.getAPI(SERVER_GETCAMPAIGNSREQUIRE+'?id='+vm.selectedCustomer.id+'&status='+vm.query.status+'&ads=GG',true, function(e){
                var isSuccess = e.success == 1;
                if(!isSuccess){
                    $mdDialog.show(
                        $mdDialog.alert()
        ...
        function openCampaign(campaign, $event, index) {
            $mdDialog.show({
                ...
                templateUrl: 'app/main/employee/gg-ads/open-ads-dialog.tmpl.html',
                preserveScope: true,
                autoWrap: true,
                skipHide: true,
                controller: 'EmpOpenGoogleAdsController',
                controllerAs: 'vm',
                locals: {
                    data: campaign,
                    index: index
